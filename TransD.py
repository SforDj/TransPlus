...
class TransEModel(object):
    ...
    def __init__(self, config, is_training):
        ...
        with tf.device("/cpu:0"):
            ...
            self.entity_trans = tf.get_variable("entity_transfer", [entity_num, entity_embedding_size], dtype=tf.float32)
            self.relation_trans = tf.get_variable("relation_transfer", [entity_num, relation_embedding_size], dtype=tf.float32)
            ...
            trans_pos_h = tf.nn.embedding_lookup(self.entity_trans, self.pos_h)
            trans_pos_r = tf.nn.embedding_lookup(self.entity_trans, self.pos_r)
            trans_pos_t = tf.nn.embedding_lookup(self.entity_trans, self.pos_t)
            ...
            trans_neg_h = tf.nn.embedding_lookup(self.entity_trans, self.pos_h)
            trans_neg_r = tf.nn.embedding_lookup(self.entity_trans, self.pos_r)
            trans_neg_t = tf.nn.embedding_lookup(self.entity_trans, self.pos_t)

            embedded_pos_h = self.calc(embedded_pos_h, trans_pos_r, trans_pos_h)
            embedded_pos_t = self.calc(embedded_pos_t, trans_pos_r, trans_pos_t)
            embedded_neg_h = self.calc(embedded_neg_h, trans_neg_r, trans_neg_h)
            embedded_neg_t = self.calc(embedded_neg_t, trans_neg_r, trans_neg_t)
